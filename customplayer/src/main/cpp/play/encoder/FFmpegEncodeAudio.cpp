...
#include "../../play_header/encoder/FFmpegEncodeAudio.h"
...
//    codec = avcodec_find_encoder_by_name("libfdk_aac");
    ...
    LOGCATE("log encode params buffer_size:%d",size);
    ...
    avcodec_fill_audio_frame(frame, codecContext->channels, codecContext->sample_fmt,
                             (const uint8_t *) frame_buffer, size, 1);
...
//    out[1] = new uint8_t[length];
    ...
    LOGCATE("打印长度:%d frameSample:%d",length,frame->nb_samples);
    const uint8_t *convert_buffer = audio_buffer;
    ret = swr_convert(swr, frame->data, frame->nb_samples, &convert_buffer, length / 4);
    if (ret < 0) {
